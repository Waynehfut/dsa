...
:::note 注
并不是说空间占用分析没有意义，在单片机、嵌入式系统开发等存储受限的情景中，空间分析同样非常重要！
:::
...
数字放到最后，我们考虑一个极端的情况，假设每个数字都需要比较最多的次数到自己的目标位置，那最多需要 $0+1+2+...+(n-1)=\frac{(n-1)(n-2)}{2}=\frac{n^2-3n-2}{2}$次
...
而是通过计算程序语句的执行次数，并舍弃常数项、低阶项来度量效率，这样的方式称之为大O表示法。
再看一个代码例子：
...
我们可以看到程序除去函数语句外的逻辑语句总计执行了$n+4$次，根据前面的约定，那这个程序的实现复杂度为$O(N+4)$，简化后为$O(N)$。但是实际代码中
